...
public abstract class RecalculateTaskScheduleAlgorithm extends AlgorithmBase {
    ...
    private void fulfilDependencies() throws TaskDependencyException {
        for (Iterator distances = myDistance2dependencyList.entrySet()
            ...
            Set nextDependenciesSet = (HashSet) nextEntry.getValue();
			for (Iterator it = nextDependenciesSet.iterator(); it.hasNext();) {
				TaskDependency nextDependency = (TaskDependency) it.next();
    ...
    private void modifyTaskStart(Task task, GanttCalendar newStart) {
        ...
        ((TaskImpl) task).isFromSuperTask = true;
        mutator.setStart(newStart);
        mutator.commit();
        ((TaskImpl) task).isFromSuperTask = false;
    ...
    private void buildDistanceGraph(TaskDependency[] deps, int distance) {
        ...
        // List会存入重复的依赖关系，导致执行fulfilDependencies()方法会有严重的性能问题；
		// 改用Set（可自动过滤掉重复的数据） Added by Owen_wen 2013-06-03
		Set depsSet = (Set) myDistance2dependencyList.get(key);
		if (depsSet == null) {
			depsSet = new HashSet();
			myDistance2dependencyList.put(key, depsSet);
		}
