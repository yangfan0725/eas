...
*/
	...
	public void setUnionData() {
		...
		amountColumns.add("acctTaxAmount");
		amountColumns.add("aimTaxAmount");
...
//		this.actionQuery.setEnabled(false);
	...
	private void fillNode(DefaultMutableTreeNode node) throws BOSException,
		...
		if (node.isLeaf()) {
			...
			BigDecimal aimTaxAmount  = (BigDecimal) this.aimCostMap.get(acctId+"_AimTaxAmount");
			row.getCell("aimTaxAmount").setValue(aimTaxAmount);
			...
			if (happenDataInfo != null) {
				...
				if(happenDataInfo.getTaxAmount()!= null){
					taxAmount = taxAmount.add(happenDataInfo.getTaxAmount());
				}
			}
			...
			row.getCell("acctTaxAmount").setValue(taxAmount);
			...
			//已实现=已完工工程量
			if(this.happenGetter.paySplitMap!=null){
