...
//					}
					...
					entry.setRateAmount((BigDecimal)row.getCell("rateAmount").getValue());
					entry.setAmountNoTax((BigDecimal)row.getCell("amountNoTax").getValue());
...
//					plTables.remove(i);
	...
	public void initTable(KDTable table, CostAccountTypeEnum type,String productId) {
		...
		column.setKey("rate");
		column = table.addColumn();
		column.setKey("rateAmount");
		column = table.addColumn();
		column.setKey("amountNoTax");
		...
		row.getCell("rate").setValue("进项税率（%）");
		row.getCell("rateAmount").setValue("进项税额");
		row.getCell("amountNoTax").setValue("不含税金额");
...
//		formattedTextField.setMaximumValue(FDCHelper.MAX_TOTAL_VALUE.multiply(FDCHelper.MAX_TOTAL_VALUE));
		...
		table.getColumn("rateAmount").getStyleAttributes().setLocked(true);
		table.getColumn("rateAmount").getStyleAttributes().setBackground(lockColor);
		
		table.getColumn("amountNoTax").getStyleAttributes().setLocked(true);
		table.getColumn("amountNoTax").getStyleAttributes().setBackground(lockColor);
		...
		formattedTextField.setNegatived(false);
		numberEditor = new KDTDefaultCellEditor(formattedTextField);
		table.getColumn("rate").setEditor(numberEditor);
...
//		table.getColumn("indexName").setEditor(editor);
	...
	private void fillNode(KDTable table, DefaultMutableTreeNode node)
		...
		row.getCell("rate").setValue(costAcct.getRate());
...
//		BigDecimal sellArea = (BigDecimal) this.apportionMap.get(key);
		...
		row.getCell("rate").setValue(info.getRate());
		row.getCell("rateAmount").setValue(info.getRateAmount());
		row.getCell("amountNoTax").setValue(info.getAmountNoTax());
	...
	protected void table_editStopped(KDTable table, KDTEditEvent e)
		...
		if (table.getColumnKey(columnIndex).equals("index")
			...
			refreshWorkload(table,row);
				...
				|| table.getColumnKey(columnIndex).equals("workload")||table.getColumnKey(columnIndex).equals("rate")) {
...
/*			放到refreshAdjustAmount(row);内计算
		...
		BigDecimal div=FDCHelper.add(new BigDecimal(1), FDCHelper.divide(row.getCell("rate").getValue(), new BigDecimal(100), 4, BigDecimal.ROUND_HALF_UP));
		row.getCell("amountNoTax").setValue(FDCHelper.divide(row.getCell("sumPrice").getValue(), div, 2, BigDecimal.ROUND_HALF_UP));
		row.getCell("rateAmount").setValue(FDCHelper.multiply(FDCHelper.divide(row.getCell("rate").getValue(), new BigDecimal(100), 4, BigDecimal.ROUND_HALF_UP), row.getCell("amountNoTax").getValue()).setScale(2, BigDecimal.ROUND_HALF_UP));
		refreshAdjustAmount(table,row);
	}
						...
						if(productType==null&&((ICell)planIndexTable.binder.getBindCellMap().get(entry.getProp()))!=null){
							...
							for(int j=planIndexTable.dynRowBase;j<planIndexTable.getTable().getRowCount()-1;j++){
								...
								if(planIndexRow.getCell(0).getValue() instanceof PlanIndexTypeEnum){
									...
									if(value==null||value.toString().trim().length()<1||productType==null){
										continue;
									}
