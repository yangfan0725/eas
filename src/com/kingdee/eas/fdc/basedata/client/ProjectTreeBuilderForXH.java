 ...
 * @version EAS7.0.1
 */
public class ProjectTreeBuilderForXH{
...
//		添加项目
	...
	private TreeModel buildOrgTree(ItemAction actionOnLoad,String curOrgId) throws Exception{
		...
		Map orgs = PermissionFactory.getRemoteInstance().getAuthorizedOrgs(
				new ObjectUuidPK(SysContext.getSysContext().getCurrentUserInfo().getId()), OrgType.Company,null, null, null);
		if (orgs != null) {
	...
	private void buildProjectTree(KDTree projectTree, TreeModel newTreeModel) throws Exception {
		...
		for(Iterator it = idNode.keySet().iterator();it.hasNext();){
			...
			String orgId = info.getCityProject().getId().toString();
	...
	public ProjectTreeBuilderForXH() {
