...
public class SupplierStockAccountReportFacadeControllerBean extends AbstractSupplierStockAccountReportFacadeControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.fdc.invite.markesupplier.rpt.SupplierStockAccountReportFacadeControllerBean");
    ...
    protected RptParams _createTempTable(Context ctx, RptParams params)    throws BOSException, EASBizException
	{
	    ...
	    header.setLabels(new Object[][]{ 
	    		...
	    		,
	    		{
	    			"所属组织","已合作/限制使用以上","已合作/限制使用以上","已合作/限制使用以上","已合作/限制使用以上","已合作/限制使用以上","已合作/限制使用以上","已合作/限制使用以上","新合作供应商","合作过且不合格供应商"
    ...
    protected String getSql(Context ctx,RptParams params){
    	...
    	sb.append(" from (select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'excellent' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId left join T_INV_MarketGradeSetUp grade on grade.fid=supplier.FGradeId where grade.fnumber='01'");
    	sb.append(dateTypeWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'qualified' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId left join T_INV_MarketGradeSetUp grade on grade.fid=supplier.FGradeId where grade.fnumber='02'");
    	sb.append(dateTypeWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'limit' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId left join T_INV_MarketGradeSetUp grade on grade.fid=supplier.FGradeId where grade.fnumber='03'");
    	sb.append(dateTypeWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'newCop' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.fid not in(select gather.fsupplierId from T_INV_MarketSRG gather left join T_INV_MarketAccreditationType type on type.fid=gather.FEvaluationTypeId where type.fnumber in('003','004','005') and gather.fstate='4AUDITTED' "+lessDateWhere.toString()+")");
    	...
    	sb.append(" )group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'copUnPass' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.FIsPass=2 and supplier.fid in(select gather.fsupplierId from T_INV_MarketSRG gather left join T_INV_MarketAccreditationType type on type.fid=gather.FEvaluationTypeId where type.fnumber='003' and gather.fstate='4AUDITTED')");
    	sb.append(dateTypeWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'unCopPass' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.FIsPass=1 and supplier.fid not in(select gather.fsupplierId from T_INV_MarketSRG gather left join T_INV_MarketAccreditationType type on type.fid=gather.FEvaluationTypeId where type.fnumber='003' and gather.fstate='4AUDITTED')");
    	sb.append(dateWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'untilLastYear' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.FIsPass=1");
    	sb.append(yearLessDateWhere);
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'new' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.FIsPass=1");
    	...
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
    	sb.append(" union all select purchaseOrgUnit.fname_l2 orgUnit,purchaseOrgUnit.flongNumber orgUnitLongNumber,'unPass' name,count(supplier.fid) amount from T_INV_MarketSupplierStock supplier left join T_ORG_Purchase purchaseOrgUnit on purchaseOrgUnit.fid=supplier.FPurchaseOrgUnitId where supplier.FIsPass=2 and supplier.fid in(select gather.fsupplierId from T_INV_MarketSRG gather left join(select gather.fsupplierId,max(gather.fbizDate) fbizDate from T_INV_MarketSRG gather where gather.fstate='4AUDITTED' group by gather.fsupplierId) gather1 on gather1.fsupplierId=gather.fsupplierId where gather.fstate='4AUDITTED' and gather1.fbizDate=gather.fbizDate and gather.FIsPass=1 "+lessDateWhere.toString()+")");
    	sb.append(" group by purchaseOrgUnit.fname_l2,purchaseOrgUnit.flongNumber");
