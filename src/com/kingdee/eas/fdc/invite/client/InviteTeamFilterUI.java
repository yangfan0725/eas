 ...
 * 招标小组自定义过滤界面
 */
public class InviteTeamFilterUI extends AbstractInviteTeamFilterUI {
	private static final long serialVersionUID = 1L;

	private static final Logger logger = CoreUIObject
			.getLogger(InquiryFileFilterUI.class);
	 ...
	 */
	public InviteTeamFilterUI() throws Exception {
		super();
	}

	public InviteTeamFilterUI(ListUI listUI, ItemAction actionListOnLoad)
			throws Exception {
		super();
		this.listUI = listUI;
...
private TreeModel getOrgTreeModel() throws OUException, Exception  {
	...
	private void initProjectDlg(String[] projectIds) throws Exception {
		...
		if (this.companySelectDlg != null
			...
			builder.buildByCostOrgTree(tree, this.companySelectDlg
					.getSelectTree());
		} else {
	...
	private void initInviteType(String inviteTypeId) throws Exception {
		...
		if (inviteTypeInstance.exists(pk)) {
			InviteTypeInfo inviteType = inviteTypeInstance
	...
	private void initCompanyDlg(String[] selectCompayIds) throws OUException,
		...
		TreeModel orgTreeModel = NewOrgUtils.getTreeModel(
				OrgViewType.COMPANY, "", cuInfo.getId().toString(), null,
				FDCHelper.getActionPK(this.actionListOnLoad));
...
//		this.companySelectDlg = null;
	...
	public FilterInfo getFilterInfo() {
		...
		if (para.isNotNull(COMPANY_IDS)) {
			...
			DefaultKingdeeTreeNode root = (DefaultKingdeeTreeNode) tree
					.getRoot();
			...
			if (set.size() > 0) {
				filter.getFilterItems().add(
						new FilterItemInfo("orgUnit.id", set,
								CompareType.INCLUDE));
			}
		}
		// 工程项目
		if (para.isNotNull(PROJECT_IDS)) {
			filter.getFilterItems().add(
					new FilterItemInfo("inviteProject.project.id", FDCHelper
		...
		if (para.isNotNull(INVITETYPE_ID)) {
			filter.getFilterItems().add(
					new FilterItemInfo("inviteProject.inviteType.id", para
							.getString(INVITETYPE_ID)));
		}
		// 文件标题
		if (para.isNotNull(NAME)) {
			filter.getFilterItems().add(
					new FilterItemInfo("name", "%"+para.getString(NAME)+"%",
							CompareType.LIKE));
		}
		// 编制日期从
		if (para.isNotNull(CREATETIME_FROM)) {
			filter.getFilterItems().add(
					new FilterItemInfo("createTime", para
							.getDate(CREATETIME_FROM),
							CompareType.GREATER_EQUALS));
		}
		// 编制日期至
		if (para.isNotNull(CREATETIME_TO)) {
			filter.getFilterItems().add(
					new FilterItemInfo("createTime", getDayEnd(para
							.getDate(CREATETIME_TO)), CompareType.LESS_EQUALS));
		}
		// 单据状态
	...
	public CustomerParams getCustomerParams() {
		...
		// 采购类型
		InviteTypeInfo inviteTypeInfo = (InviteTypeInfo) this.prmtInviteType
				.getValue();
