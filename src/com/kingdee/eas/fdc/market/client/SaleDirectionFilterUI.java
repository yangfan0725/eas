...
import com.kingdee.eas.fdc.sellhouse.client.TrackRecordStatFilterUI;
 ...
 * output class name
 */
public class SaleDirectionFilterUI extends AbstractSaleDirectionFilterUI {
	private static final Logger logger = CoreUIObject.getLogger(TrackRecordStatFilterUI.class);
	...
	private static final String IS_SHOW_PRE = "chkIsShowPre";
	 ...
	 */
	public SaleDirectionFilterUI() throws Exception {
		super();
	}

	public SaleDirectionFilterUI(ListUI listUI, ItemAction actionListOnLoad) throws Exception {
		super();
		this.listUI = listUI;
	...
	public FilterInfo getFilterInfo() {
		...
		// String compareExpr = "toPurchaseDate";
		// if(this.chkIsShowPre.isSelected())
		// compareExpr = "toSaleDate";
		...
		// FilterItemInfo(compareExpr,beginQueryDate
		// ,CompareType.GREATER_EQUALS));
		// if(endQueryDate!=null)
		// filter.getFilterItems().add(new
		// FilterItemInfo(compareExpr,endQueryDate,CompareType.LESS_EQUALS));

		filter = new FilterInfo();
		filter.getFilterItems().add(new FilterItemInfo("beginDate", new Timestamp(beginQueryDate.getTime())));
		filter.getFilterItems().add(new FilterItemInfo("endDate", new Timestamp(endQueryDate.getTime())));
		filter.getFilterItems().add(new FilterItemInfo(DATE_TYPE, new Integer(para.getInt(DATE_TYPE))));
		filter.getFilterItems().add(new FilterItemInfo(IS_SHOW_PRE, new Integer(para.getBoolean(IS_SHOW_PRE) ? 1 : 0)));
	...
	public CustomerParams getCustomerParams() {
		...
		param.add(IS_SHOW_PRE, this.chkIsShowPre.isSelected());
		param.add(IS_SHOW_SPECIAL, this.chkIsSpecialBiz.isSelected());
	...
	public void setCustomerParams(CustomerParams cp) {
		...
		this.chkIsShowPre.setSelected(para.getBoolean(IS_SHOW_PRE));
		this.chkIsSpecialBiz.setSelected(para.getBoolean(IS_SHOW_SPECIAL));
		this.initControlByDateType();
	...
	public boolean verify() {
		...
		if (beginQueryDate != null && endQueryDate != null) {
			...
			if (FDCDateHelper.dateDiff("d", beginQueryDate, endQueryDate) > 365) {
				MsgBox.showWarning(this, "查询时间间隔不能大于1年!");
				return false;
			}
		} else {
