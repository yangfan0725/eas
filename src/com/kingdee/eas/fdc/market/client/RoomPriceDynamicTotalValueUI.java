 ...
 * output class name
 */
public class RoomPriceDynamicTotalValueUI extends AbstractRoomPriceDynamicTotalValueUI {
     ...
     */
    public RoomPriceDynamicTotalValueUI() throws Exception
    {
        super();
        tblMain.checkParsed();
		...
		for(int j=hjColumn;j<tblMain.getColumnCount();j++){
			...
			if(j==tblMain.getColumnCount()-1){
    			...
    			if(latestarea.subtract(hadsellarea).subtract(hadpricearea).compareTo(BigDecimal.ZERO) != 0){
    				row.getCell("syhzjj").setValue((latestamount.subtract(hadsellamount).subtract(hadpriceamount)).divide((latestarea.subtract(hadsellarea).subtract(hadpricearea)), BigDecimal.ROUND_HALF_UP).setScale(2));
    			}
			}
		}
	...
	public void tableDataRequest(KDTDataRequestEvent kdtdatarequestevent) {
		try {
            ...
            Map hmap = getUIContext();
            float discountTemp = Float.parseFloat((String)hmap.get("discount"));
            BigDecimal discount = BigDecimal.valueOf(discountTemp).divide(HUNDRED,2,BigDecimal.ROUND_HALF_UP);
            String projectId = (String)hmap.get("projectId");
            KDTable roomPrice = (KDTable)hmap.get("kdtPriceAdjustEntry");
            StringBuilder builder = new StringBuilder();
            if(roomPrice != null && roomPrice.getRowCount()>0){
            	for(int i=0;i<roomPrice.getRowCount();i++){
            		builder.append(",'").append(roomPrice.getRow(i).getCell("roomId").getValue().toString()).append("'");
            	}
            }
    		...
    		if(projectId!=null){
    			if (!"".equals(projectId)) {
			        ...
			        sqlBuilder.clear();
			        sqlBuilder.appendSql("/*dialect*/select ftype,fproducttypeid,fnewarearange,yearbefore,sum(area) as area,count(*) as fploidy,round(avg(FBuildPrice),2) as fprice,round(sum(fdealtotalamount),2) as famount \n")
			        		  ...
			        		  .appendSql("as area,room.FBuildPrice,signman.fdealtotalamount, \n")
			        		...
			        		for(int i=0;i<this.tblMain.getRowCount();i++){
			        			...
			        			if(tempid.equals(currentRow.getCell("projectlongnumber").getValue().toString())){
			        				if("0".equals(yearbefore)){
			        					...
			        					//currentRow.getCell("beforeyearprice").setValue(price);
			        ...
			        sqlBuilder.appendSql("/*dialect*/select ftype,fproducttypeid,fnewarearange,sum(area) as area,count(*) as fploidy,round(avg(FBuildPrice),2) as fprice,round(sum(amount*(cfmanageragio/100)),2) as famount,hadPrice from( \n")
			        		  ...
			        		  .appendSql("and room.fid not in(' '").appendSql(builder.toString()).appendSql(") \n")
			        		  .appendSql("and project.fid='").appendSql(projectId).appendSql("') t ")
			        		  .appendSql("left outer join t_mar_areaset p on t.area > p.fminarea and t.area<= p.fmaxarea) m \n")
			        ...
			        sqlBuilder.clear();
			        sqlBuilder.appendSql("/*dialect*/select t.*,p.fid as fnewarearange from( \n")
			        		  .appendSql("select project.fid as projectid,type.fid fproducttypeid,property.fnumber ftype,room.fid as roomId, \n")
			        		  .appendSql("case when FIsActualAreaAudited='1' then room.factualBuildingArea \n")
			        		  .appendSql("else case when fispre='1' then room.fBuildingArea \n")
			        		  .appendSql("else case when FIsPlan='1' then room.fplanBuildingArea \n")
			        		  .appendSql("else room.fplanBuildingArea end end end as area \n")
			        		  .appendSql("from T_SHE_Room room \n")
			        		  .appendSql("left join T_SHE_BuildingProperty property on property.fid = room.fbuildingpropertyid \n")
			        		  .appendSql("left join T_SHE_Building build on build.fid=room.FBuildingID \n")
			        		  .appendSql("left join T_FDC_ProductType type on type.fid=build.fproducttypeid \n")
			        		  .appendSql("left join T_SHE_SellProject project on build.FSellProjectID=project.fid \n")
			        		  .appendSql("where build.fisgetcertificated=1 \n")
			        		  .appendSql("and room.fid in(' '").appendSql(builder.toString()).appendSql(") \n")
			        		  .appendSql("and project.fid='").appendSql(projectId).appendSql("') t ")
			        		  .appendSql("left outer join t_mar_areaset p on t.area > p.fminarea and t.area<= p.fmaxarea \n");
			        rowSet = sqlBuilder.executeQuery();
			        if(rowSet != null){
			        	String type,producttype,arearange,tempid,roomId;
			        	IRow currentRow;
			        	BigDecimal area=BigDecimal.ZERO,amount=BigDecimal.ZERO;
			        	while(rowSet.next()){
			        		area = BigDecimal.ZERO;
			        		amount = BigDecimal.ZERO;
			        		roomId = rowSet.getString("roomId");
			        		type = getValue(rowSet.getString("ftype"));
			        		if(type.equals("001")){
			        			type = "1house";
			        		}else if(type.equals("002")){
			        		...
			        		tempid = type.concat(producttype).concat(arearange);
			        		for(int i=0;i<roomPrice.getRowCount();i++){
			        			currentRow = roomPrice.getRow(i);
			        			if(roomId.equals(currentRow.getCell("roomId").getValue().toString())){
			        				//累加到已取预证已定价  preRoomArea -预测房间面积   actualRoomArea -实测房间面积  newRoomPrice -原总价
			        				if(currentRow.getCell("buildingArea").getValue() != null){
			        					area = new BigDecimal(currentRow.getCell("buildingArea").getValue().toString());
			        				}
			        				if(currentRow.getCell("newSumAmount").getValue() != null){
		        						amount = new BigDecimal(currentRow.getCell("newSumAmount").getValue().toString()).multiply(discount).setScale(2);
		        					}
			        				
			        				//累加到已取预证已定价
				        			for(int k=0;k<this.tblMain.getRowCount();k++){
					        			IRow tempIdRow = this.tblMain.getRow(k);
					        			if(tempid.equals(tempIdRow.getCell("projectlongnumber").getValue().toString())){
					        				//已取预证已定价面积
						        			BigDecimal hadpricearea = tempIdRow.getCell("hadpricearea").getValue()==null?BigDecimal.ZERO:(BigDecimal)tempIdRow.getCell("hadpricearea").getValue();
						        			//已取预证已定价金额
						        			BigDecimal hadpriceamount = tempIdRow.getCell("hadpriceamount").getValue()==null?BigDecimal.ZERO:(BigDecimal)tempIdRow.getCell("hadpriceamount").getValue();
						        			//已取预证已定价套数
						        			int hadpricecount = tempIdRow.getCell("hadpricecount").getValue()==null?0:Integer.parseInt(tempIdRow.getCell("hadpricecount").getValue().toString());
					        				
						        			tempIdRow.getCell("hadpricearea").setValue(hadpricearea.add(area));
						        			tempIdRow.getCell("hadpriceamount").setValue(hadpriceamount.add(amount.divide(WAN,BigDecimal.ROUND_HALF_UP).setScale(2)));
						        			tempIdRow.getCell("hadpricecount").setValue(String.valueOf(hadpricecount+1));
						        			if(hadpricearea.add(area).compareTo(BigDecimal.ZERO) != 0){
						        				tempIdRow.getCell("hadpriceprice").setValue((hadpriceamount.add(amount.divide(WAN,BigDecimal.ROUND_HALF_UP).setScale(2))).divide(hadpricearea.add(area), BigDecimal.ROUND_HALF_UP).setScale(2));
						        			}
					        				break;
					        			}
				        			}
