 ...
 * output class name
 */
public class QuestionStatFilterUI extends AbstractQuestionStatFilterUI {
	 ...
	 */
	public QuestionStatFilterUI(ListUI listUI, ItemAction actionListOnLoad)	throws Exception
	{
		super();
		this.listUI = listUI;
	...
	public FilterInfo getFilterInfo() {
		...
		if (para.isNotNull(IS_SHOW_ALL) && !para.getBoolean(IS_SHOW_ALL)&& para.isNotNull(DATE_TYPE)) 
		{
			...
			{
				filter.getFilterItems().add(new FilterItemInfo("date", getBeginQueryDate(para),	CompareType.GREATER_EQUALS));
			}
			if (getEndQueryDate(para) != null) 
			{
				filter.getFilterItems().add(new FilterItemInfo("date", FDCHelper.getNextDay(getEndQueryDate(para)),CompareType.LESS));
			}
		}
		else
		{
	...
	public String getDocumentId(FDCCustomerParams para)
	{
		return para.getString(DOCUMENTID);
	}
	
	public String getPaperId(FDCCustomerParams para)
	{
		return para.getString(PAPERID);
	}
	
	public String getProjectId(FDCCustomerParams para)
	{
		return para.getString(PROJECTID);
	}
	...
	public CustomerParams getCustomerParams() {
		...
		QuestionPaperDefineInfo info = (QuestionPaperDefineInfo)this.f7Question.getValue();
		
		param.add(PAPERID, info.getId().toString());
		param.add(DOCUMENTID, info.getDocumentId());
		
		SellProjectInfo project = (SellProjectInfo)this.f7SellProject.getValue();
		if(project != null && project.getId() != null)
		{
			param.add(PROJECTID, project.getId().toString());
		}
	...
	public void setCustomerParams(CustomerParams cp) {
		...
		String paperId = para.getString(PAPERID);
		if(paperId != null)
		{
			try
			{
				QuestionPaperDefineInfo info = QuestionPaperDefineFactory.getRemoteInstance().getQuestionPaperDefineInfo(new ObjectUuidPK(BOSUuid.read(paperId)));
			
				f7Question.setValue(info);
		...
		String projectId = para.getString(PROJECTID);
		if(projectId != null)
		{
			try
			{
				SellProjectInfo info = SellProjectFactory.getRemoteInstance().getSellProjectInfo(new ObjectUuidPK(BOSUuid.read(projectId)));
			
				f7SellProject.setValue(info);
			} catch (Exception e)
			{
				e.printStackTrace();
			}
	...
	public boolean verify() {
		if(this.f7Question.getValue() == null)
		{
			MsgBox.showWarning("调查问卷不能为空！");
			return false;
		}
		FDCCustomerParams para = new FDCCustomerParams(getCustomerParams());
