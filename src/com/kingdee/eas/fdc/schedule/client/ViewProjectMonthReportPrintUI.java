...
//			if (collection.size() > 0) {
	...
	public String getChartTitle() throws Exception {
		...
		if (getUIContext().get("Owner") != null && getUIContext().get("Owner") instanceof ProjectMonthReportEditUI) {
			...
			ProjectMonthReportInfo projectMonthReportInfo = (ProjectMonthReportInfo) getUIContext().get("reportInfo");
				...
				content = projectMonthReportInfo.getYear() + "年第" + projectMonthReportInfo.getMonth() + "月" + projectMonthReportInfo.getProject().getName() + "项目月报";
				String title = projectMonthReportInfo.getYear() + "年第" + projectMonthReportInfo.getMonth() + "月";
				...
				for (int i = 0; i < kdtThisEntry.getRowCount(); i++) {
					...
					String str = title + "工作共" + thisMonth.size() + "项,按时完成" + finished + "项,延时完成" + delayed + "项,待确认" + affirm + "项,延时且未完成"
							+ unfinished + "项,执行中" + excudeing + "项。计划下月工作共有" + nextMonth.size()
							+ "项,见如下饼状图：";
					Font font = new Font(str, Font.PLAIN, 12);
					content = content + "\n" + font.getName();
				} else {
					String str = title + "工作共" + thisMonth.size() + "项,按时完成0项,延时完成0项,待确认0项,延时且未完成0项,执行中0项。计划下月工作共有" + nextMonth.size() + "项," + "\n"
							+ "见如下饼状图：";
					Font font = new Font(str, Font.PLAIN, 12);
					content = content + "\n" + font.getName();
				}
	...
	public void actionExportExcel_actionPerformed(ActionEvent e) throws Exception {
		...
		tables.add(new Object[] { "本月任务", kdtThisEntry });
		tables.add(new Object[] { "下月任务", kdtNextEntrys });
