...
/**
 * @(#)							
 * 版权：		金蝶国际软件集团有限公司版权所有 描述： 主项计划达成率分析
 * @author 李兵
 * @version EAS7.0
 * @createDate 2011-8-26
 * @see
 */
public class MainRateAchievedUI extends AbstractMainRateAchievedUI
{
    private static final Logger logger = CoreUIObject.getLogger(MainRateAchievedUI.class);
     ...
     */
    public MainRateAchievedUI() throws Exception
    {
        super();
    }
	...
	private void initPreTime() {
		...
		String function = "MainRateAchievedUI";
		String treeId = null;
		try {
    ...
    protected void cbYear_itemStateChanged(java.awt.event.ItemEvent e) throws Exception
    {
		...
		for (int i = fstart; i <= fend; i++) {
			...
			if (fstc != null) {
				...
				/**
				 * 由于从数据库里面取出来的是sql包下的日期类型，但是这里的d又是util下的日期类型，
				 * 因此临界的时候出现了问题，1号的数据也取了出来。<br>
				 * 因此在这里直接将fstc的size做为累计计划完成数不妥<br>
				 * 放到下面去再次设置下
				 */
			...
			int allLateDone = 0;
			/**
			 * 这个equals就是为了排除util和sql包下的Date类型不同导致的临界的时候算累计计划完成数不正确的问题 add by
			 * libing
			 */
			int equals = 0;
			...
			// 累计实际完成数：任务状态为“已完成”且“计划完成日期”在当前月之前（包含历史年份，包含当前月）的主项任务。
			if (!row.getCell("allPlan").getValue().toString().equals("0")) {
				// 累计完成率K
				row.getCell("allRate").setExpressions("=J" + m + "/" + "I" + m);
			}
			//累计按时完成数:默认隐藏,任务状态为"已完成"且"实际完成日期"小于等于"计划完成日期"且"计划完成日期"在当前月之前(包含历史年份
			// ，包含当前月)的主项任务.
			...
			// 累计延时完成数:默认隐藏,任务状态为"已完成"且"实际完成日期"大于"计划完成日期"且"计划完成日期"在当前月之前(包含历史年份，
			// 包含当前月)的主项任务.
			// 累计延时完成数N
		...
		try {
			...
			if (exits(objs)) {
				...
				if (isMainSchedule()) {// mainscheudlerateachivement
					// 主项所属的工程项目
					autoRemember.save(userID, orgUnitID, "MainRateAchievedUI", curProjectID);
				} else {
