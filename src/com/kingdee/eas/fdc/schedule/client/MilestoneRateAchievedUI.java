 ...
 * output class name
 */
public class MilestoneRateAchievedUI extends AbstractMilestoneRateAchievedUI
{
    private static final Logger logger = CoreUIObject.getLogger(MilestoneRateAchievedUI.class);
     ...
     */
    public MilestoneRateAchievedUI() throws Exception
    {
        super();
    }
	...
	private void initPreTime() {
		...
		String function = "MileStoneAchievedUI";
		String treeId = null;
		try {
	...
	protected void initChart() {
		...
		if (node != null && node.isLeaf() && node.getUserObject() instanceof CurProjectInfo) {
			...
			if (cpinfo.getId() != null) {
				...
				// CompareType.EQUALS));
				// info.getFilterItems().add(new
				// FilterItemInfo("taskEntrys.taskType",
				...
				try {
					...
					// inf.getFilterItems().add(new
					// FilterItemInfo("schedule.id", fdcsinfo.getId()));
					// inf.getFilterItems().add(new FilterItemInfo("taskType",
					// RESchTaskTypeEnum.MILESTONE));
					// view2.setFilter(inf);
					// FDCScheduleTaskCollection ftCol =
					// FDCScheduleTaskFactory.getRemoteInstance
					// ().getFDCScheduleTaskCollection(view2);
					// if (ftCol.size() <= 0) {
					// return;
					// }
					//						
					...
					if (col != null) {
						...
						for (int count = 0; count < col.size(); count++) {
							...
							EntityViewInfo view2 = new EntityViewInfo();
							FilterInfo inf = new FilterInfo();
							inf.getFilterItems().add(new FilterItemInfo("schedule.id", fdcsinfo.getId()));
							inf.getFilterItems().add(new FilterItemInfo("taskType", RESchTaskTypeEnum.MILESTONE));
							view2.setFilter(inf);
							FDCScheduleTaskCollection ftCol = FDCScheduleTaskFactory.getRemoteInstance().getFDCScheduleTaskCollection(view2);
							if (ftCol.size() <= 0) {
								continue;
							}
    ...
    protected void cbYear_itemStateChanged(java.awt.event.ItemEvent e) throws Exception
    {
		...
		// // 当月计划完成数
		// int[] curPlan = new int[mon2 - mon1];
		// // 当月实际完成数
		// int[] curAct = new int[mon2 - mon1];
		// // 按时完成数
		// int[] aswc = new int[mon2 - mon1];
		// int[] curPlanUN = new int[mon2 - mon1];
		// // 延时完成数
		// int[] yswc = new int[mon2 - mon1];
		...
		// 根据日期过滤出该年的任务
		// update by libing at 2011-10-12 注释掉下面两行代码。跨年的时候任务不能被塞选出来，于是被注释掉~
		...
		// CompareType.EQUALS));
		finfo.getFilterItems().add(new FilterItemInfo("taskType", RESchTaskTypeEnum.MILESTONE));
		...
		for (int i = 0; i < col.size(); i++) {
			...
			// -1 第一个时期早， 0 相等 ，1第二个日期早
			int diffday2 = DateUtils.compareUpToDay(info2.getEnd(), c1.getTime());
			if (diffday2 < 0) {
				continue;
			}
			// 以上为13号修改内容
		...
		for (int i = fstart; i <= fend; i++) {
			...
			ff.getFilterItems().add(new FilterItemInfo("taskType", RESchTaskTypeEnum.MILESTONE));
			...
			for (int co = 0; co < fstc.size(); co++) {
				...
				if (scheduleTaskInfo.getComplete().compareTo(new BigDecimal(100)) < 0) {
					...
					// 已完成
					allDone += 1;
					if (scheduleTaskInfo.getActualEndDate() != null && scheduleTaskInfo.getEnd() != null) {
	...
	protected Dataset createDataset() {
		...
		for (int i = 0; i < count; i++) {
			 if (tblMain.getRow(i).getCell("wellRate").getValue() == null) {
				...
				// result.addValue(70, "当月达成率 (按时完成)", tblMain
				// .getRow(i)
				// .getCell("month").getValue().toString());
			}
	...
	public boolean destroyWindow() {
		try {
			...
			if (exits(objs)) {
				...
				autoRemember.save(userID, orgUnitID, "MileStoneAchievedUI", curProjectID);
			}
		} catch (BOSException e) {
			e.printStackTrace();
		}
