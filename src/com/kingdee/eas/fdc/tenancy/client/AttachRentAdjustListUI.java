 ...
 * output class name
 */
public class AttachRentAdjustListUI extends AbstractAttachRentAdjustListUI
{
    private static final Logger logger = CoreUIObject.getLogger(AttachRentAdjustListUI.class);
    
    SaleOrgUnitInfo saleOrg = SHEHelper.getCurrentSaleOrg();
    /**
     * output class constructor
     */
    public AttachRentAdjustListUI() throws Exception
    {
        super();
    }
	...
	protected String getEditUIName() {
		return AttachRentAdjustEditUI.class.getName();
	}
	...
	public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		AttachRentAdjustFactory.getRemoteInstance().audit(BOSUuid.read(id));
		this.refresh(null);
	}
	...
	public void actionBlankOut_actionPerformed(ActionEvent e) throws Exception {
		...
		if (MsgBox.showConfirm2New(this, "作废后不可恢复，请确认是否作废?") == MsgBox.YES) {
			...
			AttachRentAdjustInfo attachInfo = AttachRentAdjustFactory.getRemoteInstance()
			.getAttachRentAdjustInfo(new ObjectUuidPK(BOSUuid.read(id)));
			attachInfo.setState(FDCBillStateEnum.INVALID);
			SelectorItemCollection sels = new SelectorItemCollection();
			sels.add("state");
			AttachRentAdjustFactory.getRemoteInstance().updatePartial(attachInfo, sels);
...
//				info.getAttachState();
//			}
			if(AttachRentAdjustFactory.getRemoteInstance().execute(id)) {
	...
	public void actionEdit_actionPerformed(ActionEvent arg0) throws Exception {
		...
		String auditorName = (String) row.getCell("auditor.name").getValue();
		if (auditorName != null) {
			MsgBox.showInfo("配套资源调租单已经审批,不能修改!");
			return;
		}
