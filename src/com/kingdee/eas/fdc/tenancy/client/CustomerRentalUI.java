 ...
 * output class name
 */
public class CustomerRentalUI extends AbstractCustomerRentalUI
{
    private static final Logger logger = CoreUIObject.getLogger(CustomerRentalUI.class);
    private IRow footRow = null;
    private CustomerRentalFilterUI filterUI = null;
	private CommonQueryDialog commonQueryDialog = null;
    public CustomerRentalUI() throws Exception
    {
        super();
    }
...
//    	actionRefresh.setVisible(false);
    	...
    	tblMain.getColumn("dayPrice").getStyleAttributes().setHided(true);
    }
	...
	public void showDialog(final Map param){
        ...
        dialog.setLongTimeTask(new ILongTimeTask() {
        	public Object exec()throws Exception {
        		Map result = CustomerRentalFacadeFactory.getRemoteInstance().getCustomerRentalList(param);
	    		fillTable(result);
	    		return result;
           }
		...
		if (list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (customerList != null) {
					...
					//@by huanghefh住宅性质
					String buildingProperty=(String)customerList.get("buildingProperty");
					...
					//by huanghefh buildingProperty
					row.getCell("buildingProperty").setValue(buildingProperty);
	...
	private CustomerRentalFilterUI getFilterUI() {
		if (filterUI == null) {
			try {
				filterUI = new CustomerRentalFilterUI();
			} catch (Exception e) {
				handleException(e);
			}
		}
