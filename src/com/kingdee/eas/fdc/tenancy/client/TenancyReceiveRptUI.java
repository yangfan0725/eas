...
import com.kingdee.eas.fdc.tenancy.TenancyReceivingRptFacadeFactory;
 ...
 * output class name
 */
public class TenancyReceiveRptUI extends AbstractTenancyReceiveRptUI
{
    private static final Logger logger = CoreUIObject.getLogger(TenancyReceiveRptUI.class);
	private boolean isQuery =false;
    
    /**
     * output class constructor
     */
    public TenancyReceiveRptUI() throws Exception
    {
        super();
    }
    @Override
	...
	protected CommRptBaseConditionUI getQueryDialogUserPanel() throws Exception {
		// TODO Auto-generated method stub
		return new TenancyReceivingRptFilterUI();
	}

	@Override
	protected ICommRptBase getRemoteInstance() throws BOSException {
		// TODO Auto-generated method stub
		return TenancyReceivingRptFacadeFactory.getRemoteInstance();
	}
	...
	protected void query() {
                 ...
                 public void afterExec(Object result)throws Exception{
                     ...
                     KDTableUtil.setHeader(header, tblMain);
          	        ...
          	        while(rs.next()){
          	        	...
          	        	row.getCell("revAmt").setValue(rs.getBigDecimal("revAmt"));
          	        	row.getCell("revDate").setValue(rs.getObject("revDate"));
          	        	row.getCell("apAmt").setValue(rs.getBigDecimal("apAmt"));
          	        	row.getCell("revStartDate").setValue(rs.getObject("revStartDate"));
          	        	row.getCell("revEndDate").setValue(rs.getObject("revEndDate"));
          	        	row.getCell("leaseTime").setValue(rs.getBigDecimal("leaseTime"));
          	        	row.getCell("alreayTenArea").setValue(rs.getBigDecimal("tenancyArea"));
          	        }
		...
		CRMClientHelper.changeTableNumberFormat(tblMain, new String[]{"buildingArea","buildingSellArea","tenancyArea","leaseCount","freedays","totalAmt","baseDayPrice","realDayPrice","revAmt","apAmt","leaseTime"});
		CRMClientHelper.fmtDate(tblMain, new String[]{"startDate","endDate","revStartDate","revEndDate","revDate"});
...
//		}
	...
	@Override
	protected void chkNotDisNoRevData_stateChanged(ChangeEvent e)
			throws Exception {
		// TODO Auto-generated method stub
		super.chkNotDisNoRevData_stateChanged(e);
		boolean isNotDisplay = chkNotDisNoRevData.isSelected();
		int rc = this.tblMain.getRowCount();
		IRow r = null;
		for(int i=0;i<rc;i++){
			r = tblMain.getRow(i);
			if(r.getCell("revDate").getValue() == null){
				r.getStyleAttributes().setHided(isNotDisplay);
			}
		}
	}
}