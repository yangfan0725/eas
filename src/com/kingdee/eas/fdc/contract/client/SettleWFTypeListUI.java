 ...
 * output class name
 */
public class SettleWFTypeListUI extends AbstractSettleWFTypeListUI
{
    private static final Logger logger = CoreUIObject.getLogger(SettleWFTypeListUI.class);
    public SettleWFTypeListUI() throws Exception
    {
        super();
	...
	protected ITreeBase getTreeInterface() throws Exception {
		return SettleWFTypeFactory.getRemoteInstance();
	}
	...
	protected void treeMain_valueChanged(javax.swing.event.TreeSelectionEvent e) throws Exception {
		...
		if (node.getUserObject() instanceof SettleWFTypeInfo) {
			SettleWFTypeInfo cti = (SettleWFTypeInfo) node.getUserObject();
			HashSet lnUps = new HashSet();
			try {
				SettleWFTypeCollection ctc = SettleWFTypeFactory.getRemoteInstance().getSettleWFTypeCollection(
	...
	public void actionAddNew_actionPerformed(ActionEvent e) throws Exception {
		SettleWFTypeInfo cti = null;
		DefaultKingdeeTreeNode node = (DefaultKingdeeTreeNode) treeMain.getLastSelectedPathComponent();
		if (node == null) {
			return;
		} else {
			if (node.getUserObject() instanceof SettleWFTypeInfo) {
				cti = (SettleWFTypeInfo) node.getUserObject();
			}
		}
		if (cti != null) {
	...
	private boolean isHasSubLevel(SettleWFTypeInfo cti) throws EASBizException, BOSException {
		boolean flag = true;
		if (cti != null) {
			flag = SettleWFTypeFactory.getRemoteInstance().exists("select id where parent.id = '" + cti.getId().toString() + "'");
		}
		return flag;
	}
	private boolean isReferenced(SettleWFTypeInfo cti) throws EASBizException, BOSException {
		boolean flag = false;
		if (cti != null) {
			flag = ContractBillFactory.getRemoteInstance().exists("select id where SettleWFType.id = '" + cti.getId().toString() + "'");
		}
		return flag;
	}
	...
	protected String getEditUIName() {
		return SettleWFTypeEditUI.class.getName();
	}
    ...
    public void actionCancelCancel_actionPerformed(ActionEvent e) throws Exception
    {
		...
		ISettleWFType iSettleWFType = SettleWFTypeFactory.getRemoteInstance();
		IObjectPK pk = new ObjectStringPK(id);
		if (iSettleWFType.enabled(pk)) {
    ...
    public void actionCancel_actionPerformed(ActionEvent e) throws Exception
    {
		...
		ISettleWFType iSettleWFType = SettleWFTypeFactory.getRemoteInstance();
		IObjectPK pk = new ObjectStringPK(id);
		if (iSettleWFType.disEnabled(pk)) {
