 ...
 * output class name
 */
public class ContractEvaluationListUI extends AbstractContractEvaluationListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ContractEvaluationListUI.class);
    
    public ContractEvaluationListUI() throws Exception
    {
        super();
    }
	...
	protected void audit(List ids) throws Exception {
		ContractEvaluationFactory.getRemoteInstance().audit(ids);
	}
	protected void unAudit(List ids) throws Exception {
		ContractEvaluationFactory.getRemoteInstance().unAudit(ids);
	}
...
//		getBillListTable().getColumn("version").getStyleAttributes().setNumberFormat("#,##0.0;-#,##0.0");
	...
	protected boolean  displayBillByContract(KDTSelectEvent e,EntityViewInfo view) throws BOSException {
		...
		ContractEvaluationCollection contractChangeBillCollection = ContractEvaluationFactory.getRemoteInstance().getContractEvaluationCollection(view);
		for (Iterator iter = contractChangeBillCollection.iterator(); iter.hasNext();) {
			ContractEvaluationInfo element = (ContractEvaluationInfo) iter
					.next();
...
//		}
	...
	protected String getEditUIName() {
		return ContractEvaluationEditUI.class.getName();
	}
...
//		this.menuWorkFlow.setVisible(false);
	...
	public void actionModify_actionPerformed(ActionEvent e) throws Exception {
		ContractEvaluationInfo info=getSelectedInfo();
...
//		}
	...
	private void checkAudited(ContractEvaluationInfo info) throws BOSException, EASBizException {
		if (!FDCUtils.isBillAudited(info)) {
			MsgBox.showWarning(this, "非审批单据不能修订！");
	        SysUtil.abort();
		}
	}
	private void checkLastVersion(ContractEvaluationInfo info) throws BOSException, EASBizException {
//		if(!info.isIsLatest()){
//			MsgBox.showWarning(this, "非最新版本不能修订！");
//	        SysUtil.abort();
//		}
	}
	private ContractEvaluationInfo getSelectedInfo() throws BOSException, EASBizException {
		checkSelected();
		...
		sel.add("respPerson.*");
		sel.add("respDept.*");
		return ContractEvaluationFactory.getRemoteInstance().getContractEvaluationInfo(new ObjectUuidPK(getSelectedKeyValue()),sel);
	}
