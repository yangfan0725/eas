...
import com.kingdee.eas.fdc.contract.ContractInvoiceWTReportFacadeFactory;
 ...
 * output class name
 */
public class ContractInvoiceWTReportUI extends AbstractContractInvoiceWTReportUI
{
    private static final Logger logger = CoreUIObject.getLogger(ContractInvoiceWTReportUI.class);
    
    public ContractInvoiceWTReportUI() throws Exception
    {
        super();
        tblMain.checkParsed();
	...
	protected CommRptBaseConditionUI getQueryDialogUserPanel() throws Exception {
		return new ContractInvoiceWTReportFilterUI();
	}

	protected ICommRptBase getRemoteInstance() throws BOSException {
		return ContractInvoiceWTReportFacadeFactory.getRemoteInstance();
	}
		...
		tblMain.getColumn("invoiceType").setRenderer(new ObjectValueRender(){
			public String getText(Object obj) {
				if(obj instanceof String){
					...
					if(WTInvoiceTypeEnum.getEnum(info)==null){
						return "";
					}else{
						return WTInvoiceTypeEnum.getEnum(info).getAlias();
					}
				}
	...
	protected void btnValid_actionPerformed(ActionEvent e) throws Exception {
		...
		for (int i = 0; i < selectRows.length; i++) {
			...
			String invid=(String)row.getCell("invid").getValue();
			
			ContractWTInvoiceEntryInfo info=new ContractWTInvoiceEntryInfo();
			info.setId(BOSUuid.read(invid));
			info.setIsValid(true);
			ContractWTInvoiceEntryFactory.getRemoteInstance().updatePartial(info, sic);
		}
