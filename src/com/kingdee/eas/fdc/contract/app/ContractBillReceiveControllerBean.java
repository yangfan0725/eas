...
public class ContractBillReceiveControllerBean extends AbstractContractBillReceiveControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.fdc.contract.app.ContractBillReceiveControllerBean");
	...
	protected void _save(Context ctx, IObjectPK pk, IObjectValue model)
		...
		ContractBillReceiveInfo info = (ContractBillReceiveInfo)model;
		info.setSrcAmount(info.getAmount());
		info.setSignDate(info.getBookedDate());
...
//    	if(((ContractBillReceiveInfo)model).getBail()!=null){
//    		ContractBailFactory.getLocalInstance(ctx).save(((ContractBillReceiveInfo)model).getBail());
//    	}
    	
    	ContractBillReceiveInfo info = (ContractBillReceiveInfo)model;
    	
    	if(info.getId() != null&&_exists(ctx, new ObjectUuidPK(info.getId()))){
    		ContractBillReceiveInfo oldInfo = ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(new ObjectUuidPK(info.getId()));
	...
	protected IObjectPK _submit(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		ContractBillReceiveInfo ContractBillReceiveInfo = ((ContractBillReceiveInfo) model);
		//checkBillForSubmit( ctx, FDCBillInfo);
		if(ContractBillReceiveInfo.getId()!=null){
			boolean canSubMit = _checkCanSubmit(ctx, ContractBillReceiveInfo.getId().toString());
			if(!canSubMit){
				String msg = "单据状态已经在审核中或者已审核，不能再提交";
				throw new ContractException(ContractException.WITHMSG,new Object[]{msg});
			}
		}
//		if(ContractBillReceiveInfo.getBail()!=null){
//			ContractBailFactory.getLocalInstance(ctx).submit(ContractBillReceiveInfo.getBail());
//		}
		ContractBillReceiveInfo.setCodingNumber(ContractBillReceiveInfo.getNumber());
		 ...
		 * add by emanon
		 */
		checkNameDup(ctx, ContractBillReceiveInfo);
		
		//补充合同提交后,更新关联的预估合同变更为isUsed为true wangxin 20120331
//		if(ContractPropertyEnum.SUPPLY.equals(ContractBillReceiveInfo.getContractPropert())){
//			ContractBillEntryCollection coll = ContractBillReceiveInfo.getEntrys();
...
//		}
		//扣减控制金额
//		ProgrammingContractInfo proInfo = ContractBillReceiveInfo.getProgrammingContract();
//		if(proInfo!=null){
//			BigDecimal balance = proInfo.getBalance();
//			balance = balance.subtract(ContractBillReceiveInfo.getAmount());
...
//		}
		ContractBillReceiveInfo.setSrcAmount(ContractBillReceiveInfo.getAmount());
		IObjectPK pk=super._submit(ctx, ContractBillReceiveInfo);
		
		if(model.get("fromweb")==null){
    		sendtoOA(ctx,ContractBillReceiveInfo);
    	}
		return pk;
	}
	public void sendtoOA(Context ctx,ContractBillReceiveInfo info) throws EASBizException{
//    	try {
...
//			}
//			if(url!=null){
//				info=this.getContractBillReceiveInfo(ctx, new ObjectUuidPK(info.getId()));
//				
...
//					sic.add("state");
//					ContractBillReceiveFactory.getLocalInstance(ctx).updatePartial(info,sic);
//					
...
//		}
	...
	public void deleteOA(Context ctx,ContractBillReceiveInfo info) throws EASBizException{
    	if(info.getSourceFunction()!=null){
...
//		}
	 ...
	 * @param ContractBillReceiveInfo
	 */
	private void setCreatorPosition(Context ctx, ContractBillReceiveInfo ContractBillReceiveInfo) {
		...
		ContractBillReceiveInfo.setCreatorPosition(positionInfo);
	}
	...
	protected void checkBillForSubmit(Context ctx, IObjectValue model)throws BOSException, EASBizException {
		...
		//不能落于当前成本期间之前
		ContractBillReceiveInfo contractBill = (ContractBillReceiveInfo)model;
        if(model==null || contractBill.getCurProject()==null ||contractBill.getCurProject().getFullOrgUnit()==null){
        	model= this.getContractBillReceiveInfo(ctx,new ObjectUuidPK(contractBill.toString()),getSic());
        }
		//是否启用财务一体化
	...
	protected void checkBillForAudit(Context ctx, BOSUuid billId)throws BOSException, EASBizException {
		...
		ContractBillReceiveInfo ContractBillReceiveInfo = this.getContractBillReceiveInfo(ctx,new ObjectUuidPK(billId.toString()),getSic());
		//检查功能是否已经结束初始化
		String comId = ContractBillReceiveInfo.getCurProject().getFullOrgUnit().getId().toString();
			
		HashMap param = FDCUtils.getDefaultFDCParam(ctx,ContractBillReceiveInfo.getOrgUnit().getId().toString());		
...
//			splitBeforeAudit = FDCUtils.getDefaultFDCParamByKey(ctx, costCenterId, FDCConstants.FDC_PARAM_SPLITBFAUDIT);
		...
		//该合同已经还没有拆分，不能审核
		if(splitBeforeAudit && ContractBillReceiveInfo.isIsCoseSplit() &&  
				!ContractBillReceiveInfo.isIsAmtWithoutCost() && !CostSplitStateEnum.ALLSPLIT.equals(ContractBillReceiveInfo.getSplitState())){
...
//			SysUtil.abort();
		...
		boolean isInCore = FDCUtils.IsInCorporation( ctx, comId);
		if(isInCore){
			String curProject = ContractBillReceiveInfo.getCurProject().getId().toString();
			//没有结束初始化
			if(!ProjectPeriodStatusUtil._isClosed(ctx,curProject)){
				throw new ProjectPeriodStatusException(ProjectPeriodStatusException.ISNOT_INIT,new Object[]{ContractBillReceiveInfo.getCurProject().getDisplayName()});
			}			
			//不再当前期间，不能审核
		}
		//单据审批时预算控制
...
//        if (isShowCharge && ContractBillReceiveInfo.getConChargeType() != null) {
//           invokeAuditBudgetCtrl(ctx,ContractBillReceiveInfo);
//        }
	}
	...
	protected void checkBillForUnAudit(Context ctx, BOSUuid billId)throws BOSException, EASBizException {
        ...
        ContractBillReceiveInfo ContractBillReceiveInfo = this.getContractBillReceiveInfo(ctx,new ObjectUuidPK(billId.toString()),sic);
...
//			splitBeforeAudit = FDCUtils.getDefaultFDCParamByKey(ctx, costCenterId, FDCConstants.FDC_PARAM_SPLITBFAUDIT);
    	...
    	if (ContractUtil.hasChangeAuditBill(ctx, ContractBillReceiveInfo.getId())) {
    		throw new  ContractException(ContractException.HASCHANGEAUDITBILL);
    	}
    	
    	//R110603-0148如果存在变更指令单，则不允许反审批
    	if (ContractUtil.hasContractChangeBill(ctx, ContractBillReceiveInfo.getId())) {
...
//				throw new ProjectPeriodStatusException(ProjectPeriodStatusException.CLOPRO_HASEND,new Object[]{ContractBillReceiveInfo.getCurProject().getDisplayName()});
//			}		
			
			//单据期间在工程项目当前期间之前，不能反审核
			PeriodInfo costPeriod = FDCUtils.getCurrentPeriod(ctx,curProject,true);
			if(ContractBillReceiveInfo.getPeriod().getBeginDate().before(costPeriod.getBeginDate())){
				throw new  ContractException(ContractException.CNTPERIODBEFORE);
			}			
		}
		...
		boolean isShowCharge = FDCUtils.getDefaultFDCParamByKey(ctx,null,FDCConstants.FDC_PARAM_CHARGETYPE);
//        if (isShowCharge && ContractBillReceiveInfo.getConChargeType() != null) {
//           invokeUnAuditBudgetCtrl(ctx,ContractBillReceiveInfo);
//        }
	}
...
//		ContractBaseDataHelper.synToContractBaseData(ctx, false, pk.toString());
	...
	protected void setPropsForBill(Context ctx, FDCBillInfo fDCBillInfo) throws EASBizException, BOSException {
		...
		ContractBillReceiveInfo contractBill = (ContractBillReceiveInfo)fDCBillInfo;
		EntityViewInfo view = new EntityViewInfo();
		FilterInfo filter = new FilterInfo();
	...
	protected boolean _contractBillStore(Context ctx, IObjectPK ctPK, String storeNumber) throws BOSException, EASBizException {
		boolean flag = false;
//		ContractBillReceiveInfo cbi = this.getContractBillReceiveInfo(ctx,ctPK);
//		 String sql =
...
//					viewInfo.getSelector().add("splitState");
					...
					ContractBillReceiveInfo contractbill = ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(new ObjectUuidPK(mainId));
...
//					String updateSql = "update T_CON_Contractbill set FAmount=?,FlocalAmount=? where Fid=?";
		...
		//ContractBillReceiveInfo contractBill = (ContractBillReceiveInfo)this.getValue(ctx,new ObjectUuidPK(billId.toString()),seletor);
	}
...
//		ContractStateHelper.synToExecState(ctx, billId.toString(), ContractExecStateEnum.SIGNED);
		...
		if(!ContractBillReceiveFactory.getLocalInstance(ctx).
				exists(filter)){
//			ContractExecInfosFactory.getLocalInstance(ctx)
...
//	   	sel.add("*");
//	   	sel.add("entrys.*");
//	   	ContractBillReceiveInfo contractInfo =  ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(new ObjectUuidPK(billId), sel);
...
//		}
//		ContractBillReceiveInfo ContractBillReceiveInfo = this.getContractBillReceiveInfo(ctx, new ObjectUuidPK(billIdStr), getSic());
//		ContractBillReceiveInfo supplierContractBillReceiveInfo= this.getContractBillReceiveInfo(ctx, new ObjectUuidPK(billId), getSic());
...
//		ProgrammingContractInfo pcInfo = ContractBillReceiveInfo.getProgrammingContract();
//		if (pcInfo == null)
//			return;
//		// 规划余额
...
//		// 合同签约金额
//		BigDecimal signAmount = ContractBillReceiveInfo.getAmount();
...
//			}
//			if(ContractPropertyEnum.SUPPLY.equals(supplierContractBillReceiveInfo.getContractPropert())){
//				//流程里面报错放到前台？
...
////				}
//				ContractEstimateChangeBillFactory.getLocalInstance(ctx).sub(pcInfo, ContractEstimateChangeTypeEnum.SUPPLY,supAmount,true,ContractBillReceiveInfo.getOrgUnit().getId().toString());
//			}else{
...
//		}
	...
	private void updatePeriod(Context ctx, BOSUuid billId) throws EASBizException, BOSException {
		...
		ContractBillReceiveInfo billInfo = ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(new ObjectUuidPK(billId), selectors);
	...
	//单据审批时 预算控制
	private void invokeAuditBudgetCtrl(Context ctx,ContractBillReceiveInfo conBill) throws BOSException ,EASBizException{
	...
	//单据反审批时 预算控制
	private void invokeUnAuditBudgetCtrl(Context ctx,ContractBillReceiveInfo conBill) throws BOSException ,EASBizException{
	...
	protected void _unAudit(Context ctx, BOSUuid billId) throws BOSException, EASBizException {
		...
		ContractBillReceiveInfo info = (ContractBillReceiveInfo)this.getValue(ctx,new ObjectUuidPK(billId),selectors);
...
//			   Filter1.getFilterItems().add(new FilterItemInfo("contractbill.id",billId));
//		   }else if(ContractPropertyEnum.SUPPLY.equals(contractInfo.getContractPropert())){
//			   contractInfo =  ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo("select id where number = '"+contractInfo.getMainContractNumber()+"'");
...
//		}
	...
	protected boolean _contractBillStore(Context ctx, IObjectValue cbInfo, String storeNumber) throws BOSException, EASBizException {
		boolean flag = false;
		ContractBillReceiveInfo cbi = (ContractBillReceiveInfo)cbInfo;
		///////////////////////////////////////////////////////////
		//使用编码规则取号
...
//		//合同编码规则，必须合同编码作为应用属性
//		String bindingProperty = "codeType.number";
//		ContractBillReceiveInfo info = (ContractBillReceiveInfo)_getValue(ctx, pk,sic);
...
//		ContractBillReceiveInfo cbi = null;
//		if(cbc.size()!=0){
//			cbi = cbc.get(0);
//		}
//		
...
//			if(cctc.size()!=0){
//				((ContractBillReceiveInfo)cbi).setCodeType(cctc.get(0));
				////
				// 有ID，且启用了编码规则
...
//		}
	...
	{
		ContractBillReceiveInfo cbi = (ContractBillReceiveInfo)info;
	...
	private boolean setNumber(Context ctx,FDCBillInfo info, ContractBillReceiveInfo cbi, String orgId, String bindingProperty, ICodingRuleManager iCodingRuleManager) throws BOSException, EASBizException {
		if (orgId != null && orgId.trim().length() > 0
...
//				}
	...
	protected IObjectPK _addnew(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		setCreatorPosition(ctx, (ContractBillReceiveInfo) model);
...
//	   	sel.add("entrys.*");
//	    IContractEstimateChangeBill iFaced =  ContractEstimateChangeBillFactory.getLocalInstance(ctx);
//	   	ContractBillReceiveInfo contractInfo =  ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(pk, sel);
...
//		}
		...
		deleteOA(ctx,ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(pk));
...
/*			FDCSQLBuilder builder = new FDCSQLBuilder();
	...
	protected Map _fetchInitData(Context ctx, Map paramMap) throws BOSException, EASBizException {
		...
		initMap.put("contractBillInfo", null);
		return initMap;
	}
	...
	protected void  initProject(Context ctx, Map paramMap,Map initMap) throws EASBizException, BOSException{
		...
		if(projectId == null && paramMap.get("contractBillId")!=null) {
			...
			ContractBillReceiveInfo contractBill = ContractBillReceiveFactory.getLocalInstance(ctx).
			getContractBillReceiveInfo(new ObjectUuidPK(BOSUuid.read(contractBillId)),selector);
...
//        sic.add(new SelectorItemInfo("splitState"));
//        sic.add(new SelectorItemInfo("orgUnit.id"));
//		ContractBillReceiveInfo ContractBillReceiveInfo = this.getContractBillReceiveInfo(ctx,pk,sic);
//		
//		HashMap param = FDCUtils.getDefaultFDCParam(ctx,ContractBillReceiveInfo.getOrgUnit().getId().toString());		
//		boolean splitBeforeAudit = false;
//		if(param.get(FDCConstants.FDC_PARAM_SPLITBFAUDIT)!=null){
...
//		return CostSplitStateEnum.ALLSPLIT.equals(ContractBillReceiveInfo.getSplitState());
	}
	...
	protected boolean _autoDelSplit(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		...
		ContractBillReceiveInfo ContractBillReceiveInfo = 
			ContractBillReceiveFactory.getLocalInstance(ctx).getContractBillReceiveInfo(pk,sic);
		if(FDCBillStateEnum.AUDITTED.equals(ContractBillReceiveInfo.getState())){
...
/*		builder.appendSql("delete from T_Con_ContractCostSplitEntry where fparentid in (select fid from T_Con_ContractCostSplit where fcontractbillid=?)");
	...
	protected String _getContractTypeNumber(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		...
		return ContractBillReceiveInfo.getContractType().getLongNumber().replace('!','.');
	}
	...
	protected void checkNameDup(Context ctx, FDCBillInfo billInfo) throws BOSException, EASBizException {
		ContractBillReceiveInfo contractBill = (ContractBillReceiveInfo) billInfo;
		FilterInfo filter = new FilterInfo();
	...
	protected void _cancel(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		...
		SelectorItemCollection selector = new SelectorItemCollection();
		selector.add("state");
		ContractBillReceiveInfo billInfo = ContractBillReceiveFactory.getLocalInstance(ctx).
		getContractBillReceiveInfo(pk,selector);
	    ...
	    try{
	    	while(rs.next()&&rs.size()==1){
	    		...
	    		ContractBillReceiveInfo supInfo= ContractBillReceiveFactory.getLocalInstance(ctx)
						.getContractBillReceiveInfo(new ObjectUuidPK(BOSUuid.read(supId)), selector);
...
//		SelectorItemCollection sel=new SelectorItemCollection();
//		sel.add("amount");
//		ContractBillReceiveInfo ContractBillReceiveInfo = this.getContractBillReceiveInfo(ctx, new ObjectUuidPK(billId), sel);
//		IProgrammingContract service = ProgrammingContractFactory.getLocalInstance(ctx);
...
//		BigDecimal amount = ContractBillReceiveInfo.getAmount();
//		
...
//				ContractBillReceiveInfo supply=new ContractBillReceiveInfo();
...
//				}
//				ContractBillReceiveFactory.getLocalInstance(ctx).updatePartial(supply, supplySel);
//				
