...
public class MarketProjectReportFacadeControllerBean extends AbstractMarketProjectReportFacadeControllerBean
{
    ...
    protected RptParams _createTempTable(Context ctx, RptParams params)    throws BOSException, EASBizException
	{
	    ...
	    initColoum(header,col,"isSub",100,true);
	    initColoum(header,col,"state",100,true);
...
//	    			"id","营销立项单据编号","立项审批通过时间","立项事项","费用归口口径","立项金额","签约单位/个人","单据类型","conId","合同审批通过时间","合同编号","合同名称","签约单位/个人","合同总金额","累计付款金额","剩余未付款金额","营销费用归属月份","营销费用发生额（含分摊）"
    ...
    protected String getSql(Context ctx,RptParams params){
    	...
    	Date fromConBizDate = (Date)params.getObject("fromConBizDate");
    	Date toConBizDate =   (Date)params.getObject("toConBizDate");
    	boolean isContract=params.getBoolean("isContract");
    	boolean isWithoutContract=params.getBoolean("isWithoutContract");
    	boolean isJZ=params.getBoolean("isJZ");
    	...
    	sb.append(" left join (select '合同' type,con.fstate,con.fid conId,con.fmarketProjectId,con.FMpCostAccountId,con.fauditTime conAuditTime,con.fIsTimeOut isTimeOut,con.fnumber conNumber,(case when con.fhasSettled=0 then '否' else '是' end) conHasSettled, con.fname conName,supplier.fname_l2 conPartB,(case when t.fsettleprice is null then con.famount else t.fsettleprice end) conAmount,entry.fdate conBizdate,(case when t.fsettleprice is null then entry.famount else t.fsettleprice*entry.frate/100 end)  conMarketAmount,p.amount payAmount from");
...
//    	sb.append(" left join T_CON_ContractBill con1 on entry.fheadid=con.fid where con.fstate!='1SAVED'");
    	...
    	sb.append(" union all select '无文本' type,con.fstate,con.fid conId,con.fmarketProjectId,con.FMpCostAccountId,con.fauditTime conAuditTime,con.fIsTimeOut isTimeOut,con.fnumber conNumber,'否' conHasSettled,con.fname conName,(case when supplier.fname_l2 is null then p.fname_l2 else supplier.fname_l2 end) conPartB,con.famount conAmount,entry.fdate conBizdate,entry.famount conMarketAmount,p.amount payAmount from");
    	...
    	if(fromBizDate!=null){
			sb.append(" and m.fbizDate>={ts '" + FDCConstants.FORMAT_TIME.format(FDCDateHelper.getSQLBegin(fromBizDate))+ "'}");
		}
		if(toBizDate!=null){
			sb.append(" and m.fbizDate<{ts '"+FDCConstants.FORMAT_TIME.format(FDCDateHelper.getSQLEnd(toBizDate))+ "'}");
		}  
		if(fromConBizDate!=null){
			sb.append(" and (case when cost.ftype='JZ' then m.fbizDate else t.conBizdate end)>={ts '" + FDCConstants.FORMAT_TIME.format(FDCDateHelper.getSQLBegin(fromConBizDate))+ "'}");
		}
		if(toConBizDate!=null){
			sb.append(" and (case when cost.ftype='JZ' then m.fbizDate else t.conBizdate end)<{ts '"+FDCConstants.FORMAT_TIME.format(FDCDateHelper.getSQLEnd(toConBizDate))+ "'}");
		}
		if(isContract||isWithoutContract||isJZ){
			sb.append(" and cost.ftype in "+type);
		}
