 ...
 * @author kelvin_yang
 * @work 房地产目标成本测算成本取数
 * @method fdc_aim_measure_cost 若成本科目未录入，返回工程项目目标成本测算该版本的成本汇总数，
 * 							若成本科目录入，则返回工程项目该目标成本测算该版本上该成本科目的成本
 */
public class FdcAimMeasureCostCaculator implements IMethodBatchQuery,
		ICalculator {
	...
	public boolean batchQuery(Map methods) {
		//获得从界面传递过来的相关参数
		SortedParameterArray params = (SortedParameterArray) methods.get("fdc_aim_measure_cost");
		if (params != null) {
			for (int i = 0; i < params.size(); i++) {
				...
				try {
					//通过计算公式计算，返回结果
					BigDecimal amount = this.fdc_aim_measure_cost(
	 ...
	 * @param verNo 目标成本测算版本号
	 * @return
	 * @throws ParseException
	 * @throws BOSException
	 * @throws SQLException
	 */
	public BigDecimal fdc_aim_measure_cost(String fdcCompanyNumber,String prjNumber,String acctLongNumber,String verNo) throws ParseException, BOSException, SQLException
	{
