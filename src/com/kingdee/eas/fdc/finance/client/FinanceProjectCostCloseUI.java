...
import com.kingdee.eas.fdc.finance.FinanceCostClosePeriodFacadeFactory;
 ...
 * output class name
 */
public class FinanceProjectCostCloseUI extends AbstractFinanceProjectCostCloseUI
{
    private static final Logger logger = CoreUIObject.getLogger(FinanceProjectCostCloseUI.class);
    private boolean isOk = false;

    /**
     * output class constructor
     */
    public FinanceProjectCostCloseUI() throws Exception
    {
        super();
    }
    ...
    public void actionConfirm_actionPerformed(ActionEvent e) throws Exception
    {
    	try{
	    	...
	    	if(rbUnClosePeriod.isSelected() &&
	    			!FDCUtils.IsUnInCorporation(null, SysContext.getSysContext().getCurrentFIUnit().getId().toString())){
	    		MsgBox.showWarning(this, "本公司不允许进行财务成本反月结！");
	    		SysUtil.abort();
	    	}
	    	...
	    	if(rbClosePeriod.isSelected()){
	    		String result = FinanceCostClosePeriodFacadeFactory.getRemoteInstance().traceFinanceCostClose(projectIdList);
	    		txtResult.setText(result);
	    	}else if(rbFrozenPeriod.isSelected()){
	    		FinanceCostClosePeriodFacadeFactory.getRemoteInstance().frozenProject(projectIdList);
	    		txtResult.setText("所选工程项目单据已冻结。");
	    	}else if(rbUnClosePeriod.isSelected()){
	//    		MsgBox.showInfo(this, "暂不提供此功能，待月结完善之后再提供！");
	//    		SysUtil.abort();
	    		String result = FinanceCostClosePeriodFacadeFactory.getRemoteInstance().antiCostClose(projectIdList);
	    		txtResult.setText(result);
	    	}else{
...
//		disposeUIWindow();
    ...
    public void onLoad() throws Exception {
    	...
    	this.acionAddAll.setEnabled(true);
    	this.acionDelAll.setEnabled(true);
