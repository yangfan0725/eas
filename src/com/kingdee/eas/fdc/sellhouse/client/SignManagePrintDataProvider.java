...
public class SignManagePrintDataProvider extends FDCBillDataProvider{
	private static final Logger logger = Logger.getLogger(SignManagePrintDataProvider.class);
	private HashMap map = null;
	public SignManagePrintDataProvider(String billId, IMetaDataPK mainQuery) {
		super(billId, mainQuery);
	}
	public SignManagePrintDataProvider(String billId,IMetaDataPK mainQuery,HashMap map)
	{
		super(billId, mainQuery);
		this.map = map;
	}
		...
		if (ds.getID().equalsIgnoreCase("PayListEntry")) {
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory.getRemoteInstance(new MetaDataPK(
						"com.kingdee.eas.fdc.sellhouse.app.SignPayListEntryPrintQuery"));
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory
						.getRemoteInstance(new MetaDataPK("com.kingdee.eas.fdc.sellhouse.app.SignCustomerEntryPrintQuery"));
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				filter.getFilterItems().add(new FilterItemInfo("head.id", billId));
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory
						.getRemoteInstance(new MetaDataPK("com.kingdee.eas.fdc.sellhouse.app.SignSaleManPrintQuery"));
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				filter.getFilterItems().add(new FilterItemInfo("head.id", billId));
