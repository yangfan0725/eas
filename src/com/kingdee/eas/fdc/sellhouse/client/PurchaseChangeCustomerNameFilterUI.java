 ...
 * output class name
 */
public class PurchaseChangeCustomerNameFilterUI extends AbstractPurchaseChangeCustomerNameFilterUI
{
    private static final Logger logger = CoreUIObject.getLogger(PurchaseChangeCustomerNameFilterUI.class);
     ...
     */
    public PurchaseChangeCustomerNameFilterUI() throws Exception
    {
        super();
    }
    
    public PurchaseChangeCustomerNameFilterUI(ListUI listUI, ItemAction actionListOnLoad) throws Exception {
		super();
		this.listUI = listUI;
	...
	public FilterInfo getFilterInfo() {
		...
		if (para.isNotNull(IS_SHOW_ALL) && !para.getBoolean(IS_SHOW_ALL)
			...
			if (getBeginQueryDate(para) != null) {
				filter.getFilterItems().add(
						new FilterItemInfo("bizDate", getBeginQueryDate(para),
								CompareType.GREATER_EQUALS));
			}
			if (getEndQueryDate(para) != null) {
				filter.getFilterItems().add(
						new FilterItemInfo("bizDate", FDCHelper
		...
		{
			filter.getFilterItems().add(new FilterItemInfo("bizDate",FDCDateHelper.getFirstYearDate(1900),CompareType.GREATER));
		}
		return filter;
	}
