...
public class NewFDCRoomPromptDialog extends KDCommonPromptDialog {
	...
	private  NewFDCRoomPromptDialog()
	{
	}
	/**
	 * 
	 ...
	 */
	public NewFDCRoomPromptDialog(Boolean isMultiSelect, BuildingInfo buildingInfo, BuildingUnitInfo buildUnit,MoneySysTypeEnum moneySysTypeEnum, RoomCollection roomColl)
	{
	...
	public NewFDCRoomPromptDialog(Boolean isMultiSelect, BuildingInfo buildingInfo, BuildingUnitInfo buildUnit,MoneySysTypeEnum moneySysTypeEnum, RoomCollection roomColl,SellProjectInfo sellProjectInfo)
	{
		this.isMultiSelect = isMultiSelect;
		this.buildingInfo = buildingInfo;
		this.buildUnit = buildUnit;
	...
	public NewFDCRoomPromptDialog(IUIObject owner)
	{
		this.owner = owner;
	}
	...
	public NewFDCRoomPromptDialog(IUIObject owner, CompanyOrgUnitInfo company)
	{
		this.owner = owner;
		companyInfo = company;
	}
	...
	public void show()
	{
		...
		try
		{
			...
			if (isRefresh || classDlg == null)
			{
				...
				classDlg = uiFactory.create(RoomSelectUIForNewSHE.class.getName(),map,null,"VIEW");
			}
			//确保每次显示isCanceled=true
			if (classDlg != null && classDlg.getUIObject() != null)
			{
				((RoomSelectUIForNewSHE)classDlg.getUIObject()).isCanceled=true;
			}
			classDlg.show();
	...
	public boolean isCanceled()
	{
        ...
        if(classDlg!=null)
        {
            return((RoomSelectUIForNewSHE)classDlg.getUIObject()).isCanceled ;
        }
        else
        {
            return true;
	...
	{
		RoomSelectUIForNewSHE ui = (RoomSelectUIForNewSHE) classDlg.getUIObject();
		Object[] returnValue;
		try
		{
			//房间多选（还没实际验证过）
