 ...
 * output class name
 */
public class TrackRecordStatFilterUI extends AbstractTrackRecordStatFilterUI
{
     ...
     */
    public TrackRecordStatFilterUI() throws Exception
    {
        super();
    }
	
    public TrackRecordStatFilterUI(ListUI listUI, ItemAction actionListOnLoad) throws Exception
    {
        super();
		this.listUI = listUI;
	...
	public Date getEndQueryDate(FDCCustomerParams para) {
		...
		return getNextDayBeginTime(DateTimeUtils.truncateDate(date));
	}
	...
	public boolean verify() {
		...
		if (beginQueryDate != null && endQueryDate != null) {
			if (!endQueryDate.after(beginQueryDate)) {
				MsgBox.showWarning(this, "结束日期必須大于开始日期!");
				return false;
			}
		}else if(!isShowall){
