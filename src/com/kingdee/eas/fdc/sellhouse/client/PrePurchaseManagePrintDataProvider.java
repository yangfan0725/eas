...
public class PrePurchaseManagePrintDataProvider extends FDCBillDataProvider{
	
	private static final Logger logger = Logger.getLogger(PrePurchaseManagePrintDataProvider.class);
	private HashMap map = null;
	public PrePurchaseManagePrintDataProvider(String billId, IMetaDataPK mainQuery) {
		super(billId, mainQuery);
	}
	public PrePurchaseManagePrintDataProvider(String billId,IMetaDataPK mainQuery,HashMap map)
	{
		super(billId, mainQuery);
		this.map = map;
	}
		...
		if (ds.getID().equalsIgnoreCase("PayListEntry")) {
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory.getRemoteInstance(new MetaDataPK(
						"com.kingdee.eas.fdc.sellhouse.app.PrePayListEntryPrintQuery"));
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory
						.getRemoteInstance(new MetaDataPK("com.kingdee.eas.fdc.sellhouse.app.PreCustomerEntryPrintQuery"));
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				filter.getFilterItems().add(new FilterItemInfo("head.id", billId));
			...
			try {
				IQueryExecutor exec = QueryExecutorFactory
						.getRemoteInstance(new MetaDataPK("com.kingdee.eas.fdc.sellhouse.app.PrePurchaseSaleManPrintQuery"));
				exec.option().isAutoTranslateEnum = true;
				EntityViewInfo ev = new EntityViewInfo();
				FilterInfo filter = new FilterInfo();
				filter.getFilterItems().add(new FilterItemInfo("head.id", billId));
