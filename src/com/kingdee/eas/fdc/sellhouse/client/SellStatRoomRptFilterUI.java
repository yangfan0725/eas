...
public class SellStatRoomRptFilterUI extends AbstractSellStatRoomRptFilterUI {
	...
	public SellStatRoomRptFilterUI(ListUI listUI, ItemAction actionListOnLoad) throws Exception {
		super();
		this.listUI = listUI;
	...
	public FilterInfo getFilterInfo() {
		...
		if(!para.getBoolean(IS_SHOW_ALL)) {
			...
			if(para.getBoolean(IS_PRE_INTO_SALE)) 
				dateParam = "toSaleDate";
			if(beginQueryDate!=null)				
				filter.getFilterItems().add(new FilterItemInfo(dateParam,beginQueryDate,CompareType.GREATER_EQUALS));
			if(endQueryDate!=null)
				filter.getFilterItems().add(new FilterItemInfo(dateParam, endQueryDate,CompareType.LESS));
		}
		
		if(!para.getBoolean(IS_INCLUDE_ATTACH))
	...
	public boolean isShowAll(FDCCustomerParams para)
	{
		return para.getBoolean(IS_SHOW_ALL);
	}
