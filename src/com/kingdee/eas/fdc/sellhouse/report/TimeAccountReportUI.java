...
import com.kingdee.eas.fdc.sellhouse.client.SHEReceiveBillListUI;
 ...
 * output class name
 */
public class TimeAccountReportUI extends AbstractTimeAccountReportUI
{
    private static final Logger logger = CoreUIObject.getLogger(TimeAccountReportUI.class);
     ...
     */
    public TimeAccountReportUI() throws Exception
    {
        super();
        tblMain.checkParsed();
	...
	protected ICommRptBase getRemoteInstance() throws BOSException {
		return TimeAccountReportFacadeFactory.getRemoteInstance();
	}
	...
	protected void query() {
		...
		CRMClientHelper.changeTableNumberFormat(tblMain, new String[]{"preArea","preAccount","preAverage","quitPreArea","quitPreAccount","quitPreAverage","purArea"
	...
	public void sumRow(IRow sumRow,IRow row,String name){
		SHEManageHelper.addSumColoum(sumRow,row,new String[]{name+"Amount",name+"Area",name+"Account"});
		sumRow.getCell(name+"Average").setValue((sumRow.getCell(name+"Area").getValue()==null||new BigDecimal(sumRow.getCell(name+"Area").getValue().toString()).compareTo(FDCHelper.ZERO)==0?FDCHelper.ZERO:((BigDecimal)sumRow.getCell(name+"Account").getValue()).divide((BigDecimal)sumRow.getCell(name+"Area").getValue(), 2,BigDecimal.ROUND_HALF_UP)));
	}
			...
			if(id.indexOf("'")<0){
				for(int j=0;j<i;j++){
					...
					if(sumRow.getCell("sellProjectId").getValue().toString().indexOf(id)>0&&sumRow.getCell("sellProjectId").getValue().toString().indexOf("'")==0){
						...
						sumRow(sumRow,row,"pre");
				    	sumRow(sumRow,row,"quitPre");
				    	sumRow(sumRow,row,"pur");
				    	sumRow(sumRow,row,"signPre");
				    	sumRow(sumRow,row,"unSignPur");
				    	sumRow(sumRow,row,"signPur");
				    	sumRow(sumRow,row,"directSign"); 
				    	sumRow(sumRow,row,"quitPur");
				    	sumRow(sumRow,row,"sign");
				    	sumRow(sumRow,row,"quitSign");
				    	sumRow(sumRow,row,"monthSign");
				    	
				    	sumRow(sumRow,row,"yearSign");
				    	sumRow(sumRow,row,"accUnSignPre");
				    	sumRow(sumRow,row,"accUnSignPur");
				    	sumRow(sumRow,row,"accSign");
				    	
				    	SHEManageHelper.addSumColoum(sumRow,row,new String[]{"compensateAmount","compensateArea","compensateAccount","compensateAreaTotal","compensateAccountTotal","signBackAccount","mcompensateAmount","mcompensateArea","mcompensateAccount","mcompensateAreaTotal","mcompensateAccountTotal","monthSignBackAccount","ycompensateAmount","ycompensateArea","ycompensateAccount","ycompensateAreaTotal","ycompensateAccountTotal","yearSignBackAccount","accSignBackAccount","accSignReceAccount","accSignReceAccountT","accSignReceAccountY"});
					}
				}
